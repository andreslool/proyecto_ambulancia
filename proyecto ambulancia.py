...
def agregar():
	...
	try:
		...
		messagebox.showwarning("BBDD", "¡Debes crear la base de datos primero!")
...
def borrarusuario():
	...
	try:
		valor=messagebox.askquestion("Borrar Usuario", "¿Estas seguro De Borrar este Usuario?")
		if valor=="yes":
			miConexion=sqlite3.connect("Emergencia")
			miCursor=miConexion.cursor()
...
def borrarllamada():
	...
	try:
		valor=messagebox.askquestion("Borrar Usuario", "¿Estas seguro De Borrar este Registro?")
		if valor=="yes":
			miConexion=sqlite3.connect("Emergencia")
			miCursor=miConexion.cursor()
...
labelregistrotitulo.grid(row=1, column=0, columnspan=2, sticky="s", padx=10, pady=0)
...
labelregistroparentesco= Label(miframe3, text="Parentesco con la emergencia: ")
